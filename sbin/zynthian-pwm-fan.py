...
def measure_temp():
    ...
    # strip off `C and convert to float
    flt_tmp = float(raw_tmp.replace("'C", ""))
    # convert to int since all temps are whole degrees only
    int_tmp = int(flt_tmp)
    return int_tmp
...
try:
    if __name__ == '__main__':
        ...
        while not TimeToQuit.now:
            ...
            smooth_speed = not_below(
                fan_min, smooth_speed - (smooth_beta * (smooth_speed - speed_x)))
